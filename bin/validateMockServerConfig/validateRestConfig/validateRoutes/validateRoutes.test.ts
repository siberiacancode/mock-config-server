...
describe('validateRoutes (rest)', () => {
  ...
  test('Should correctly handle get|delete method entities only with correct type', () => {
    ...
    incorrectEntities.forEach((incorrectEntity) => {
      ...
      ], 'get')).toThrow(new Error(`routes[0].entities.${incorrectEntity}`));
      expect(() => validateRoutes([
        {
          entities: { [incorrectEntity]: { key: 'value' } },
          data: null
        }
      ], 'delete')).toThrow(new Error(`routes[0].entities.${incorrectEntity}`));
    });
  });
  ...
  test('Should correctly handle post|put|patch method entities only with correct type', () => {
    ...
    correctEntities.forEach((correctEntity) => {
      ...
      ], 'post')).not.toThrow(Error);
      expect(() => validateRoutes([
        {
          entities: { [correctEntity]: { key: 'value' } },
          data: null
        }
      ], 'put')).not.toThrow(Error);
      expect(() => validateRoutes([
        {
          entities: { [correctEntity]: { key: 'value' } },
          data: null
        }
      ], 'patch')).not.toThrow(Error);
    });
    ...
    incorrectEntities.forEach((incorrectEntity) => {
      ...
      ], 'post')).toThrow(new Error(`routes[0].entities.${incorrectEntity}`));
      expect(() => validateRoutes([
        {
          entities: { [incorrectEntity]: { key: 'value' } },
          data: null
        }
      ], 'put')).toThrow(new Error(`routes[0].entities.${incorrectEntity}`));
      expect(() => validateRoutes([
        {
          entities: { [incorrectEntity]: { key: 'value' } },
          data: null
        }
      ], 'patch')).toThrow(new Error(`routes[0].entities.${incorrectEntity}`));
    });
  });
  ...
  test('Should correctly handle headers entity only with correct type', () => {
    ...
    incorrectHeadersValues.forEach((incorrectHeaderValue) => {
      ...
      ], 'get')).toThrow(new Error('routes[0].entities.headers'));
    });
  ...
  test('Should correctly handle params entity only with correct type', () => {
    ...
    incorrectParamsValues.forEach((incorrectParamValue) => {
      ...
      ], 'get')).toThrow(new Error('routes[0].entities.params'));
    });
    ...
    incorrectParamsObjectValues.forEach((incorrectParamsObjectValue) => {
      ...
      ], 'get')).toThrow(new Error('routes[0].entities.params.key'));
    });
  });
  ...
  test('Should correctly handle query entity only with correct type', () => {
    ...
    incorrectQueryValues.forEach((incorrectQueryValue) => {
      ...
      ], 'get')).toThrow(new Error('routes[0].entities.query'));
    });
    ...
    incorrectQueryObjectValues.forEach((incorrectQueryObjectValue) => {
      ...
      ], 'get')).toThrow(new Error('routes[0].entities.query.key'));
      expect(() => validateRoutes([
        {
          entities: { query: { key: [incorrectQueryObjectValue] } },
          data: null
        }
      ], 'get')).toThrow(new Error('routes[0].entities.query.key[0]'));
    });
  });
});
